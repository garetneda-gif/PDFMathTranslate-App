...
- **自动分批**：大文档自动按页拆分，逐批翻译后无缝合并，峰值内存从 >12 GB 降至 ~2 GB
- **Apple Silicon GPU 加速**：默认启用 CoreML 推理，版面解析速度提升 3–8 倍
...
> **注意**：首次打开时 macOS 会提示"无法验证开发者"，在**系统设置 → 隐私与安全性**中点击**仍然打开**即可。
...
处理大型学术 PDF（100 页以上）时，pdf2zh 引擎会将整份文档加载进内存，在内存有限的设备上容易触发 OOM 或系统换页，导致速度大幅下降。
...
用户文件 (300页)
    ...
    │
    ├─► 逐批 spawnPdf2zh()       ← 每次只有一个 pdf2zh 进程，处理完即退出释放内存
    │
...
pdf2zh 的进度信息通过 tqdm 输出到 stderr，以流的形式传入 Node.js。Pipe 缓冲区可能将一行进度数据切成多个 `data` 事件，也可能将多行合并为一个 `data` 事件。
...
> **背景**：BabelDOC 曾因环境特定的初始化错误[临时禁用了 CoreML](https://github.com/funstory-ai/BabelDOC/issues/170)。本应用在进程环境变量层面重新启用，在 Apple Silicon 上经验证降级行为稳定可靠。

主进程监听子进程输出，检测 CoreML 激活标志并通知渲染进程更新 UI：
...
进度区域将显示 GPU 加速徽章，让用户直观确认加速已生效。

**性能对比**（MacBook Pro M3 Pro，100 页学术 PDF，OpenAI gpt-4o-mini）：
...
不同翻译引擎（OpenAI、DeepSeek、智谱等）的 API Key、Base URL、模型名称独立存储，切换引擎时自动保存旧的、加载新的，互不覆盖：
